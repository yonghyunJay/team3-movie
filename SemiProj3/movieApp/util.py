...
def random_poster_urls(num):
    movie_url_file_path = settings.MOVIE_URL_FILE_PATH
...
def get_poster_urls(movie_list):
    movie_url_file_path = settings.MOVIE_URL_FILE_PATH
    df = pd.read_csv(movie_url_file_path)
    new_df = pd.DataFrame(columns=['name', 'img_url'])
    for item in movie_list:
        new_df = new_df.append(pd.DataFrame(df[df['name'] == item], columns=['name', 'img_url']))
...
def get_naver_id_to_list(movie_list):
    ...
    movie_url_file_path = settings.MOVIE_URL_FILE_PATH
    df = pd.read_csv(movie_url_file_path)
    new_df = pd.DataFrame(columns=['name', 'naver_url'])
    for item in movie_list:
        new_df = new_df.append(pd.DataFrame(df[df['name'] == item], columns=['name', 'naver_url']))
